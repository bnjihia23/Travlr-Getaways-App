export const trips = [
    {
    "code": "GR001",
    "name": "Gale Reef",
    "length": "7 nights",
    "start": "2025-10-15T00:00:00.000Z",
    "resort": "Blue Lagoon Resort",
    "perPerson": "1500",
    "image": "reef1.jpg",
    "description": [
      "Gale Reef — dynamic JSON proof.",
      "Sed et augue lorem. In sit amet placerat arcu. Mauris volutpat ipsum ac justo mollis vel vestibulum orci gravida. Vestibulum sit amet porttitor odio. Nulla facilisi. Fusce at pretium felis.",
      "Sed consequat libero ut turpis venenatis ut aliquam risus semper. Etiam convallis mi vel risus pretium sodales. Etiam nunc lorem ullamcorper vitae laoreet."
    ]
  },
  {
    "code": "DR002",
    "name": "Dawson’s Reef",
    "length": "5 nights",
    "start": "2025-11-05T00:00:00.000Z",
    "resort": "Coral Cove",
    "perPerson": "1200",
    "image": "reef2.jpg",
    "description": [
      "Dawson’s Reef — dynamic JSON proof.",
      "Integer magna leo, posuere et dignissim vitae, porttitor at odio. Pellentesque a metus nec magna placerat volutpat. Nunc nisi mi, elementum sit amet aliquet quis, tristique quis nisl. Curabitur odio lacus, blandit ut hendrerit.",
      "Vulputate, vulputate at est. Morbi aliquet viverra metus eu consectetur. In lorem dui, elementum sit amet convallis ac, tincidunt vel sapien."
    ]
  },
  {
    "code": "CR003",
    "name": "Claire’s Reef",
    "length": "6 nights",
    "start": "2025-12-01T00:00:00.000Z",
    "resort": "Sunset Bay",
    "perPerson": "1400",
    "image": "reef3.jpg",
    "description": [
      "Claire’s Reef — dynamic JSON proof.",
      "Donec sed felis risus. Nulla facilisi. Donec a orci tellus, et auctor odio. Fusce ac orci nibh, quis semper arcu. Cras orci neque, euismod et accumsan ac, sagittis molestie lorem.",
      "Proin odio sapien, elementum at tempor non. In hac habitasse platea dictumst. Integer purus justo, egestas eu consectetur eu, cursus in tortor. Quisque nec nunc ac mi ultrices iaculis."
    ]
  }
]